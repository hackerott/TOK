...
#while datetest <= enddate:
	...
	if os.path.isfile(filename) == True:
                windpath.append("http://breki.belgingur.is/figuras/01WRFD20101"+datetest.strftime('%Y%m%d%H')+".jpg")

        filename = "/var/www/html/figuras/02WRFD20101"+datetest.strftime('%Y%m%d%H')+".jpg"
        if os.path.isfile(filename) == True:
                temppath.append("http://breki.belgingur.is/figuras/02WRFD20101"+datetest.strftime('%Y%m%d%H')+".jpg")

        filename = "/var/www/html/figuras/03WRFD20101"+datetest.strftime('%Y%m%d%H')+".jpg"
        if os.path.isfile(filename) == True:
                rainpath.append("http://breki.belgingur.is/figuras/03WRFD20101"+datetest.strftime('%Y%m%d%H')+".jpg")

        filename = "/var/www/html/figuras/04WRFD20101"+datetest.strftime('%Y%m%d%H')+".jpg"
        if os.path.isfile(filename) == True:
                radiationpath.append("http://breki.belgingur.is/figuras/04WRFD20101"+datetest.strftime('%Y%m%d%H')+".jpg")

        filename = "/var/www/html/figuras/05WRFD20101"+datetest.strftime('%Y%m%d%H')+".jpg"
        if os.path.isfile(filename) == True:
                humiditypath.append("http://breki.belgingur.is/figuras/05WRFD20101"+datetest.strftime('%Y%m%d%H')+".jpg")
